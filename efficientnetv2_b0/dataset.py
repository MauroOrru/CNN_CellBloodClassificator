...
class BloodCellDataset(Dataset):
    def __init__(self, root_dir, valid_folders=None, img_size=400, transform=None):
        ...
        self.class_images = self._load_images_per_class() # Dictionary with class name as key
    ...
    # Private method that returns a dictionary with the class name as key and a list of images as value
    def _load_images_per_class(self):
        ...
        for class_name in os.listdir(self.root_dir): # List of folders in the root_dir
            ...
            if os.path.isdir(class_path): # Check if the path is a folder
                imgs = []
                for f in os.listdir(class_path):
                    if f.lower().endswith(('.png', '.jpg', '.jpeg', '.tiff')):
                        full_path = os.path.join(class_path, f)
                        imgs.append(full_path) # Add the image path to the list
                if imgs:
