...
MODEL_PATH = "vit_b_16_weights.pth"  # Percorso per salvare i pesi
...
class BloodCellDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if img is None:
            ...
            img = np.zeros((224, 224, 3), dtype=np.uint8)  # Vision Transformer accetta 224x224

        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img = cv2.resize(img, (224, 224))  # Rescale per Vision Transformer
...
# ---- VISION TRANSFORMER (EVITA DOWNLOAD CONTINUO) ----
def get_vit_model(num_classes, model_path=MODEL_PATH):
    ...
    # Modifica l'ultimo layer per adattarlo al numero di classi
