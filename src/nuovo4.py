...
class ConformerEEG(nn.Module):
    def __init__(self, input_dim=118, seq_len=875, cnn_embed_dim=64, d_model=128,
        ...
        Args:
            ...
            nhead         : numero di teste per l'attenzione
            num_blocks    : numero di blocchi Conformer (modificato a 2 per contenere la memoria)
...
def main():
    ...
    batch_size = 50       # Batch size ridotto
    learning_rate = 1e-4
    num_epochs = 50
    num_classes = 2
    ...
    model = ConformerEEG(
        ...
        d_model=128,
        nhead=8,
        num_blocks=2,     # ridotto per contenere la memoria
    ...
    # Scheduler: riduce il lr se la loss di validazione non migliora
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "best_conformer_eeg.pth")
        else:
            trigger_times += 1
            print(f"EarlyStopping trigger count: {trigger_times}/{patience}")
            if trigger_times >= patience:
                print("Early stopping!")
                break

    # Carica il modello migliore per valutazione finale
    model.load_state_dict(torch.load("best_conformer_eeg.pth"))
    val_loss, val_acc, all_preds, all_labels = evaluate_model(model, test_loader, criterion, device)
